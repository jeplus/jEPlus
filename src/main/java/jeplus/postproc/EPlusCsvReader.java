...
public class EPlusCsvReader implements IFResultReader {
    ...
    public int readResult(String dir, String job_id, HashMap<String, Integer> header, ArrayList<ArrayList<String>> table) {
        ...
        try (BufferedReader fr = new BufferedReader (new FileReader (job_dir + CsvSpecs.getSourceCsv()))) {            
            ...
            boolean found_report = true, found_for = true, found_table = true, found_column = false, found_row = false;
            int ColId = 0;
            String Cell = "-";
            if (CsvSpecs.getFromReport() != null && CsvSpecs.getFromReport().trim().length()>0) {
                found_report = false;
            }
            if (CsvSpecs.getFromFor()!= null && CsvSpecs.getFromFor().trim().length()>0) {
                found_for = false;
            }
            ...
            for (String [] row : spreadsheet) {
                if (found_report) {
                    if (found_for) {
                        ...
                        if (row.length > 1 && 
                            row[0]!=null && row[0].equalsIgnoreCase("FOR:") &&
                            row[1]!=null && row[1].equalsIgnoreCase(CsvSpecs.getFromFor())) {
                                found_for = true;
                        }
                    }
                }else {
