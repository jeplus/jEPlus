...
/**
 * *
 * Expression parser lang: Java code, author: Jos de Jong, 2010-01-31 site: www.speqmath.com
 *
 * Features: Operators: & | << >> = <> < > <= >= + - / % || ^ !
 *
 * Functions: Abs, Exp, Sign, Sqrt, Log, Log10 Sin, Cos, Tan, ASin, ACos, ATan Factorial
 *
 * Variables: Pi, e you can define your own variables
 *
 * Other: Scientific notation supported Error handling supported
 *
 */
import java.util.*;
...
public class Parser {
    ...
    String parse(final String new_expr) {
        try {
            ...
            if (token_type == TOKENTYPE.DELIMETER && expr_c == '\0') {
                throw new SimpleParserError(row(), col(), 4);
            }
            ...
            if (token_type != TOKENTYPE.DELIMETER || token.length() > 0) {
                if (token_type == TOKENTYPE.DELIMETER) {
                    ...
                    throw new SimpleParserError(row(), col(), 101, token);
                } else {
                    throw new SimpleParserError(row(), col(), 5, token);
                }
            }
    ...
    /**
     * parses and evaluates the given expression, and return a double value. On error, an exception is thrown.
     */
    public double resolve(final String new_expr) throws SimpleParserError {
            // initialize all variables
            expr = new_expr;     // copy the given expression to expr
            ans = 0.0;
    ...
    /**
     * Get the next character from the expression. The character is stored into the char expr_c. If the end of the expression is
     * reached, the function puts zero ('\0') in expr_c.
     */
    void getChar() {
        expr_pos++;
    ...
    /**
     * Get the first character from the expression. The character is stored into the char expr_c. If the end of the expression is
     * reached, the function puts zero ('\0') in expr_c.
     */
    void getFirstChar() {
        expr_pos = 0;
     ...
     */
    void getToken() throws SimpleParserError {
        token_type = TOKENTYPE.NOTHING;
        ...
        throw new SimpleParserError(row(), col(), 1, token);
    }

    /**
     * assignment of variable or function
     */
    double parse_level1() throws SimpleParserError {
        if (token_type == TOKENTYPE.VARIABLE) {
            ...
            if (expr_c == '=') {
                ...
                if (isLegalVariableName(var_name)) {
                    ...
                    throw new SimpleParserError(row(), col(), 300);
                }
                return ans;
            }
        }
     ...
     */
    double parse_level2() throws SimpleParserError {
        OPERATOR op_id;
        double ans;
        ans = parse_level3();
     ...
     */
    double parse_level3() throws SimpleParserError {
        OPERATOR op_id;
        double ans;
        ans = parse_level4();
     ...
     * add or subtract
     */
    double parse_level4() throws SimpleParserError {
        OPERATOR op_id;
        double ans;
        ans = parse_level5();
     ...
     */
    double parse_level5() throws SimpleParserError {
        OPERATOR op_id;
        double ans;
        ans = parse_level6();
     ...
     * power
     */
    double parse_level6() throws SimpleParserError {
        OPERATOR op_id;
        double ans;
        ans = parse_level7();
     ...
     * Factorial
     */
    double parse_level7() throws SimpleParserError {
        OPERATOR op_id;
        double ans;
        ans = parse_level8();
     ...
     * Unary minus
     */
    double parse_level8() throws SimpleParserError {
        double ans;
     ...
     * functions
     */
    double parse_level9() throws SimpleParserError {
        String fn_name;
        double ans;
     ...
     */
    double parse_level10() throws SimpleParserError {
    ...
    double parse_number() throws SimpleParserError {
        ...
        switch (token_type) {
            ...
            default:
                ...
                if (token.length() == 0) {
                    throw new SimpleParserError(row(), col(), 6);
                } else {
                    throw new SimpleParserError(row(), col(), 7);
                }
        }
     ...
     */
    double eval_operator(final OPERATOR op_id, final double lhs, final double rhs) throws SimpleParserError {
        switch (op_id) {
            // level 2
            case AND:
            ...
            case DIVIDE:
                return lhs / rhs;
            case MODULUS:
                return ParsableFunctions.modulus(lhs, rhs);
            case XOR:
                return (int) lhs ^ (int) rhs;
            ...
            // level 7
            case FACTORIAL:
                return ParsableFunctions.factorial(lhs);
        }

        throw new SimpleParserError(row(), col(), 104);
    }

    /**
     * evaluate a function
     */
    double eval_function(final String fn_name, final double value) throws SimpleParserError {
        ...
        if (fnUpper.equals("SIGN")) {
            return ParsableFunctions.sign(value);
        }
        ...
        if (fnUpper.equals("FACTORIAL")) {
            return ParsableFunctions.factorial(value);
        }

        // unknown function
        throw new SimpleParserError(row(), col(), 102, fn_name);
    }

    /**
     * evaluate a variable
     */
    double eval_variable(final String var_name) throws SimpleParserError {
        ...
        // unknown variable
        throw new SimpleParserError(row(), col(), 103, var_name);
    }
