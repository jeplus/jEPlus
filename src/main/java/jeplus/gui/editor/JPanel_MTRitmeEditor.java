 ...
 * @author Yi
 */
public class JPanel_MTRitmeEditor extends javax.swing.JPanel {

    /** Logger */
    final static Logger logger = LoggerFactory.getLogger(JPanel_MTRitmeEditor.class);

    JEPlusFrameMain MainGUI = null;
    JTree HostTree = null;
    protected JEPlusProjectV2 Project = null;
    protected RVX_MTRitem Mtr = null;
     ...
     * @param mtr
     */
    public JPanel_MTRitmeEditor(JEPlusFrameMain frame, JTree tree, JEPlusProjectV2 prj, RVX_MTRitem mtr) {
        initComponents();
        MainGUI = frame;
        HostTree = tree;
        Project = prj;
            ...
            public void insertUpdate(DocumentEvent e) {
                if (DLActive) {
                    ...
                    if(src == DocVariables) {
                        ...
                        Mtr.setMeters(Arrays.asList(vars));
                    }else if (src == DocResultTable) {
                        Mtr.setTableName(txtResultTable.getText().trim());
                    }
    ...
    protected final void setMtrItem (RVX_MTRitem mtr) {
        Mtr = mtr;
        txaVariables.setText(String.join("\n", Mtr.getMeters()));
        cboFrequency.setSelectedItem(RVX_RVIitem.Frequencies.valueOf(Mtr.getFrequency()));
        txtResultTable.setText(Mtr.getTableName());
        chkAggregate.setSelected(Mtr.isUsedInCalc());
    }
    ...
    private void cboFrequencyActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cboFrequencyActionPerformed
        if (DLActive) {
            Mtr.setFrequency(cboFrequency.getSelectedItem().toString());
            Project.setContentChanged(true);
            HostTree.update(HostTree.getGraphics());
        }
    ...
    private void chkAggregateActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_chkAggregateActionPerformed
        if (DLActive) {
            Mtr.setUsedInCalc(chkAggregate.isSelected());
            Project.setContentChanged(true);
            HostTree.update(HostTree.getGraphics());
        }
